...
## Notebooks
- Always use executable `.py` files over interactive notebooks (e.g., Jupyter notebooks `.ipynb`, marimo, etc.)
...
This repo frequently works by `git clone`-ing someone else's repo into a subdirectory and re-running their own build/pipeline inside it. This creates a mix of "brownfield" (pre-existing, vendored) and "greenfield" (new, ours) code.

- **Greenfield** (code under `GREENFIELD/`, new scripts, new pipelines you create): follow every convention above from the start — per-directory `AGENTS.md`, `.env` handling, `ASSETS/` for data, `CONFIGS/` for configs. No exceptions, no "migrate later."
- **Brownfield** (existing vendored folders): don't do a disruptive big-bang migration or refactor of someone else's repo. Concretely:
